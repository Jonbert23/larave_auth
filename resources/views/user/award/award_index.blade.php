...
@section('content')
@include('user.award.award_modal')
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <ol class="activity-feed mb-0">
                        ...
                        @foreach($award as $awards)
                            @if($awards->user_id == Auth::user()->id)
                                <li class="feed-item">
                                    <div class="float-right p-3">
                                        <a href="#" class="text-danger" data-toggle="modal" data-target="#delete_award" onclick="delete_award({{ $awards->id }})"><i class="mdi mdi-delete-forever icons "></i></a>
                                        <a href="javascript:void(0)" class="text-info" data-toggle="modal" data-target="#edit_award" onclick="edit_award({{ $awards->id }})"><i class="mdi mdi-briefcase-edit icons"></i></a>
                                    </div>
                                    <div class="feed-item-list">
                                        <span class="date text-white-50" style="font-size: 18px; font-weight: bold">{{ $awards->award_title}}</span>
                                        <span class="activity-text text-white">{{ $awards->company_name }}</span><br>
                                        <span class="activity-text text-white">{{ $awards->year_from." - ". $awards->year_to}}</span>
                                    </div>
                                </li>
        ...
        <div class="col-md-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="form-horizontal m-t-20" action="/award" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                        <a class="latest-message-list">
                            <div class="border-bottom mt-3 position-relative">
                                <div class="float-left user mr-3">
                                    <h5 class="bg-info text-center rounded-circle text-white mt-0">AT</h5>
                                </div>
                                
                                <div class="massage-desc">
                                    <h5 class="font-14 mt-0 text-dark">Award Title</h5>
                                    <input class="form-control" type="text" placeholder="Enter Award Title" name="award" style="border:none" value="{{ old('award') }}">
                                </div>
                            </div>
                        </a>
                        @error('award')
                        ...
                        <a class="latest-message-list">
                            <div class="border-bottom mt-3 position-relative">
                                ...
                                <div class="massage-desc">
                                    ...
                                    <input class="form-control" type="text" placeholder="Enter Company Name" name="company_name" value="{{ old('company_name') }}" style="border:none">
                                </div>
...
@section('script')
    <script>
        ...
        {
            $.get('/award/edit/'+id, function(award)
            {
                $("#id").val(award.id);
                $("#award").val(award.award_title);
                $("#company_name").val(award.company_name);
                $("#year_from").val(award.year_from);
                $("#year_to").val(award.year_to);
            })
        }
    </script>
